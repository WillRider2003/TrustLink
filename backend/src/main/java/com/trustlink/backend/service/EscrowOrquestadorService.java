...
public class EscrowOrquestadorService {
    ...
    public OrdenEscrow crearOrdenDeCompra(Usuario comprador, Long productoId) {
        ...
        OrdenEscrow guardada = ordenRepository.save(orden);
        auditoriaService.registrar(comprador, TipoAccionAuditoria.ORDEN_CREADA, "OrdenEscrow", guardada.getId(),
                "Compra creada por " + comprador.getEmail() + " sobre el producto \"" + producto.getNombre() + "\"");
        notificacionService.notificarCompraHecha(comprador, producto.getNombre(), codigo,
                "/marketplace/orden/" + guardada.getId());
        return guardada;
    }
    ...
    public OrdenEscrow confirmarEntrega(Usuario comprador, Long ordenLocalId, String codigoIngresado) {
        ...
        auditoriaService.registrar(comprador, TipoAccionAuditoria.ORDEN_ENTREGA_CONFIRMADA, "OrdenEscrow", orden.getId(),
                "Entrega confirmada por " + comprador.getEmail() + " para la orden #" + orden.getId());
    ...
    /**
     * El vendedor marca la orden como enviada, opcionalmente adjuntando
     * una foto de evidencia (empaque, guía, etc.). Esto NO toca el
     * contrato — el dinero sigue en custodia hasta que el comprador
     * confirme la entrega con su código. Es solo informativo y dispara
     * la notificación al comprador.
     */
    @Transactional
    public OrdenEscrow marcarComoEnviado(Usuario vendedor, Long ordenLocalId, MultipartFile foto) {
        OrdenEscrow orden = ordenRepository.findById(ordenLocalId)
                .orElseThrow(() -> new CompraException("Orden no encontrada"));

        if (!orden.getVendedor().getId().equals(vendedor.getId())) {
            throw new CompraException("No tienes permiso sobre esta orden");
        }
        if (orden.getEstado() != EstadoOrden.EN_CUSTODIA) {
            throw new CompraException("Esta orden ya no está en custodia");
        }
        if (orden.isMarcadoEnviado()) {
            throw new CompraException("Esta orden ya fue marcada como enviada");
        }

        if (foto != null && !foto.isEmpty()) {
            try {
                String rutaFoto = archivoService.guardarImagen(foto, "entregas");
                orden.setFotoEntregaUrl(rutaFoto);
            } catch (Exception e) {
                throw new CompraException("No se pudo guardar la foto de entrega: " + e.getMessage(), e);
            }
        }
        orden.setMarcadoEnviado(true);
        orden.setFechaEnvio(LocalDateTime.now());
        OrdenEscrow guardada = ordenRepository.save(orden);

        notificacionService.notificarCompraEnviada(orden.getComprador(), orden.getProducto().getNombre(),
                "/marketplace/orden/" + orden.getId());
    ...
    public OrdenEscrow reportarDisputa(Usuario usuario, Long ordenLocalId) {
        ...
        OrdenEscrow guardada = ordenRepository.save(orden);
        auditoriaService.registrar(usuario, TipoAccionAuditoria.ORDEN_DISPUTA_REPORTADA, "OrdenEscrow", orden.getId(),
                "Disputa reportada por " + usuario.getEmail() + " sobre la orden #" + orden.getId());
        return guardada;
    }
